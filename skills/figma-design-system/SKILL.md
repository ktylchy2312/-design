...
15. **Check [references/tokens-built.md](references/tokens-built.md) before creating a color/spacing/radius/typography token** — it's the ground truth for what already exists in this file's `Color`/`Spacing`/`Radius`/`Breakpoints` collections and `Type/*` text styles. Don't recreate a token that's already there under a different name.
