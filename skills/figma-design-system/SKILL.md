...
13. **Wrap output in a titled page frame, not loose canvas objects — at creation time, not as a cleanup pass after.** Every component or foundations catalog entry gets a dark header (title + one-line subtitle) over a light content area — see [references/page-presentation.md](references/page-presentation.md) for the verified structure and script pattern. This rule existed while all 28 OC-DS components were first built and got skipped for every one of them — caught only when the user compared the bare components against the (correctly wrapped) foundations pages and asked why they looked different. Fixing 28 components after the fact cost more calls than wrapping each at creation would have.
